                                ...
                                try {
                                    ...
                                    xmlGuild.onreadystatechange = () => {
                                        ...
                                        try {
                                            ...
                                            .addField('Stats', '```ml\n' + `General Information:\n- IGN: ${resTextStats.data[i].ign}\n- Date joined: ${resTextStats.data[i].dateJoined}\n- Region: ${resTextStats.data[i].region}\n- Ingame Rank: ${resTextGuild.members[w].rank}\n- Shore Leave: ${resTextStats.data[i].sl}\n\nRoles:\n${resTextStats.data[i].ahh == true? '- AHH \n': ""}${resTextStats.data[i].pillager == true? '- Pillager \n' : ""}${resTextStats.data[i].potsct == true? '- PPOTSCT \n': ""}\nMilestones:\n- GXP ${gxp[0] == null? "0":gxp[0]}: ${resTextStats.data[i].currentGXP.toLocaleString("en")} (${gxp[1].toLocaleString("en") == "Max milestone reached" ?  "Max milestone reached, overflow: " + (resTextStats.data[i].currentGXP - 10000000000).toLocaleString("en"):gxp[1].toLocaleString("en") + ' left to reach the next milestone'})\n- EMS ${ems[0] == null?"0":ems[0]}: ${resTextStats.data[i].currentEMS.toLocaleString("en")} (${ems[1].toLocaleString("en") == "Max milestone reached" ? "Max milestone reached, overflow: " + (resTextStats.data[i].currentEMS - 5000000).toLocaleString("en"):ems[1].toLocaleString("en") + ' left to reach the next milestone'})\n` + '```')
